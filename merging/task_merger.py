...
class TaskMerger(nn.Module):
    ...
    def add_task_parameters(self, base_model, parameters, concat_across_output = True, scaling_coeffs=1.):
        ...
        for key, val in parameters.items():
            try:
                ...
                base_shape = tuple(sd[key].shape) if key in sd else None
                raise RuntimeError(
                    f"failed to add the merged delta for {key!r} "
                    f"(base shape {base_shape}, delta shape {tuple(val.shape)}, "
                    f"concat_across_output={concat_across_output}): {e}"
                ) from e
        return base_model
    ...
    def matrix_to_state_dict(self, matrix, state_dict, remove_keys=[],
        ...
        if isinstance(matrix, list):
            return [self.matrix_to_state_dict(m, state_dict, remove_keys,
                                              concat_across_output) for m in matrix]
        ...
        for layer_name, value in matrix.items():
            try:
                ...
                if 'other' in parameter_types:
                    ...
                    target_shape = tuple(reference_dict[name].shape)
                    if not concat_across_output:
                        # V-sharing merges in the transposed frame, so `value` is
                        # (in, out). Keep it there and let add_task_parameters
                        # transpose it back; reshaping straight to (out, in)
                        # would scramble every non-square module.
                        target_shape = target_shape[::-1]
                    merged_state_dict[name] = value.reshape(*target_shape)
            except Exception as e:
                raise RuntimeError(
                    f"failed to convert merged matrix {layer_name!r} back to a "
                    f"state dict (matrix shape {tuple(value.shape)}): {e}"
                ) from e
...
class SVDMerger(TaskMerger):
    ...
    def transform(self, merge_config):
        ...
        if merge_config.get('ingredients_path') is not None:
            print(f"[transform] saving ingredients to {merge_config['ingredients_path']}")
            torch.save(self.ingredients, merge_config['ingredients_path'])
            print("[transform] ingredients saved")
    
    def merge(self, merge_config):
        print("[merge] loading ingredients")
        if self.ingredients is not None:
            print("[merge] using in-memory ingredients")
            ingredients = self.ingredients
        elif merge_config.get('ingredients_path') is not None:
            print(f"[merge] loading ingredients from disk: {merge_config['ingredients_path']}")
            ingredients = torch.load(merge_config['ingredients_path'], map_location='cpu')
        else:
        ...
        print("[merge] reconstructing merged parameter matrices")
        merged_sd = self.reconstruct_merged_sd(U, merged_sV_sd)
        
        merging_fn = lambda x: get_merging_fn(merge_config['merge_method'])(
            x, **merge_config, weights=self.scaling_coeffs
        )
        # Unused in this release (no config sets merge_other_params) and broken
        # as written: VectorOps.forward takes a third positional merge_config,
        # so this call would raise TypeError. Left for reference only.
        if merge_config.get('merge_other_params', False):
            print("[merge] merging non-transform parameters")
        ...
        print("[merge] converting matrices back to state dict")
        merged_sd = self.matrix_to_state_dict(
            merged_sd, ptm_reference_params,
            concat_across_output=merge_config.get('concat_across_output', True),
        )
        # Add merged sd to the ptm
        print("[merge] cloning base model")
        merged_base = deepcopy(self.pretrained_model)
        print("[merge] applying merged delta to base model")
