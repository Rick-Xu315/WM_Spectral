...
# NOTE: merging_functions.topk_values_mask is a near-duplicate of this function
# (same maths) but returns (values, density, mask) instead of the
# (values, mask, density) order used here. Both are live -- this one on the
# KnOTS/SVD path, that one on the plain-TIES path -- so do not cross-import.
