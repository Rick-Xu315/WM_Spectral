...
## TIES MERGING UTILS
# NOTE: masking_ops.topk_values_mask is a near-duplicate of this function
# (same maths) but returns (values, mask, density) instead of the
# (values, density, mask) order used here. Both are live -- this one on the
# plain-TIES path, that one on the KnOTS/SVD path -- so do not cross-import.
...
def resolve_sign(Tensor, mode='sum_of_values'):
    # Mirrors masking_ops.resolve_sign, including that only the sum_of_values
    # branch resolves zero signs. Previously this ignored `mode` and hardcoded
    # the sum_of_values behaviour, so sign_resolve_mode was a no-op here.
...
def ties_merging(vectors, topK=10, merging_type='mean', weights=None,
                 sign_resolve_mode='sum_of_values', **kwargs):
